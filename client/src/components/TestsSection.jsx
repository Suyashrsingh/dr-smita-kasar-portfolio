...
const TestsSection = ({ tests = [] }) => {
  ...
  return (
    <section id="tests" className="py-12 relative bg-slate-50/50 dark:bg-navy-950/50 rounded-3xl border border-slate-200/80 dark:border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {(!tests || tests.length === 0) ? (
          <div className="glass-card rounded-2xl p-12 text-center space-y-3 border border-slate-200/80 dark:border-slate-800/80 max-w-lg mx-auto">
            <FileCheck className="w-12 h-12 text-slate-400 mx-auto opacity-50" />
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">No Assessments Published</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              There are currently no active online assessments or quizzes published. Please check back later.
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-200/60 dark:border-white/10">
                      ...
                      <div>
                        <strong>Total MCQs:</strong> {test.questions?.length || 0} Questions
                      </div>
                      <div>
                        <strong>Passing Score:</strong> {test.passMarks} Marks
                      </div>
                      <div>
                        <strong>Target:</strong> {test.targetClass || 'All Classes'}
                      </div>
                    </div>

                    {test.isScheduled && (
                      <div className="p-3 rounded-2xl bg-academic-500/5 border border-academic-500/20 text-xs space-y-1">
                        <div className="flex items-center gap-1.5 font-semibold text-slate-700 dark:text-slate-300">
                          <Calendar className="w-3.5 h-3.5 text-academic-500" />
                          <span>Examination Schedule</span>
                        </div>
                        <div className="text-[11px] text-slate-500 font-mono">
                          {test.startTime && <div>Opens: {new Date(test.startTime).toLocaleString()}</div>}
                          {test.endTime && <div>Closes: {new Date(test.endTime).toLocaleString()}</div>}
                        </div>
                      </div>
                    )}
                  </div>

                  <div className="pt-6 border-t border-slate-200/80 dark:border-white/10 flex items-center justify-between gap-3">
                    <span className="text-xs text-slate-500 font-mono flex items-center gap-1">
                      <GraduationCap className="w-3.5 h-3.5 text-academic-500" />
                      <span>{test.targetAudience || 'Scholars'}</span>
                    </span>
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-md ${
                        sched.disabled
                          ...
                          : 'bg-academic-600 hover:bg-academic-700 text-white shadow-academic-600/25 cursor-pointer transform hover:-translate-y-0.5'
                      }`}
                    >
                      <Play className="w-3.5 h-3.5 fill-current" />
                      <span>{sched.disabled ? 'Unavailable' : 'Start Assessment'}</span>
                    </button>
                  </div>
                </div>
              );
